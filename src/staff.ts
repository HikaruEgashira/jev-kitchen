...
function staffStateOf(g: Pick<GameState, 'staffState'>, id: string): StaffState {
  const state = g.staffState?.[id];
  return {
    worked: Number.isSafeInteger(state?.worked) && (state?.worked ?? 0) >= 0 ? state.worked : 0,
    rest: Number.isSafeInteger(state?.rest) && (state?.rest ?? 0) >= 0 ? state.rest : 0,
  };
}

export function staffAvailable(
  state: Record<string, Partial<StaffState>> | undefined,
  id: string,
): boolean {
  return Object.hasOwn(STAFF, id) && state?.[id]?.rest === 0;
}

export function payroll(duty: string[] | undefined): number {
...
export function nextStaffState(
  g: Pick<GameState, 'level' | 'hired' | 'duty' | 'staffState'>,
): Record<string, StaffState> {
  const duty = new Set(Array.isArray(g.duty) ? g.duty : []);
  return Object.fromEntries(
    [
      ...new Set(
        [...(g.hired ?? []), levelConfig(g.level + 1).partner].filter((id): id is string => !!id),
      ),
    ]
...
export function nextDuty(g: Pick<GameState, 'level' | 'hired' | 'duty' | 'staffState'>): string[] {
...
const PERFORMANCE_RANGE: Record<string, { min: number; max: number }> = Object.freeze(
  Object.fromEntries(
...
export function staffPerformance(id: string): { key: string; label: string; ratio: number }[] {
  const profile = STAFF[id];
  if (!profile) return [];
