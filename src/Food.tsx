...
import type { RoundedBoxProps } from '@react-three/drei/core/RoundedBox';
import type { ThreeElements } from '@react-three/fiber';

type MeshProps = ThreeElements['mesh'];

const Layer = createContext<number | undefined>(undefined);
const palette = { white: '#fff9e8' };

interface BlockProps extends Omit<RoundedBoxProps, 'args'> {
  size: [number, number, number];
  color: string;
}

export function Block({ size, color, position, radius = 0.06, ...props }: BlockProps) {
...
export function Ball({
  ...
  scale,
  ...props
}: MeshProps & { size?: number; color: string }) {
  const order = useContext(Layer);
  return (
    <mesh
      renderOrder={order}
...
export function Cylinder({
  ...
  return (
    <mesh
      ...
      {...props}
    >
      <cylinderGeometry args={[radii[0], radii[1], height, 24]} />
...
export function Tomato({ position = [0, 0, 0], scale = 1, color = '#e85945' }: TomatoProps) {
  return (
...
export function Food({ item, burnt = false }: { item: string; burnt?: boolean }) {
...
const PREVIEW_ITEM: Record<string, string> = {
  crate: 'tomato',
  plates: 'plate',
  pot: 'soup',
  grill: 'roast',
