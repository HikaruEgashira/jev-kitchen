import { STAFF, staffAvailable, nextDuty, payroll } from './staff.ts';
import {
...
import { trainingMultiplier, trainingState } from './training.ts';
import type {
  ActionResult,
  Actor,
  Candidate,
...
export { levelConfig, quotaForLevel, MAX_LEVEL } from './progression.ts';
export {
  equipmentBurnMultiplier,
  equipmentDurationMultiplier,
  equipmentState,
...
export const STATIONS: Record<string, StationSlot> = {
...
// Recipe a station kind produces; used by advice and rule-based picking.
const RECIPE_OF_KIND: Record<string, RecipeId> = {
  board: 'dish',
  pot: 'soup',
  grill: 'roast',
};
...
export function recommendedStock(g: GameState): number {
...
function stockAmount(value: unknown): number {
  const amount = Number(value);
  return Number.isFinite(amount) ? Math.max(0, Math.floor(amount)) : 0;
}

function nextRecipe(g: GameState): RecipeId {
...
function order(g: GameState, deadline: number, recipe: RecipeId = nextRecipe(g)) {
  const id = g.nextOrder++;
  return { id, recipe, deadline, duration: deadline - g.time };
}

function orderWindowMs(g: GameState): number {
  return levelConfig(g.level).orderWindowMs;
}

function initialOrderDeadline(g: GameState, index: number): number {
  const window = orderWindowMs(g);
  return g.time + (index === 0 ? window : Math.round(window * (4 / 3)));
}

function syncConfig(g: GameState): number {
...
function resolveWho(g: GameState, who?: string | null): string {
  return who ?? g.duty?.[0] ?? '';
}
...
function makeBoardStation(): StationState {
  return {
    state: 'idle',
    busyUntil: 0,
    by: null,
...
function makeHeatStation(): StationState {
  return {
    state: 'idle',
    busyUntil: 0,
    by: null,
...
function canOperate(g: GameState, who: string, capability: Capability): boolean {
  return who === 'human' || Boolean(staffProfile(g, who)?.capabilities?.includes(capability));
}

function capabilityFailure(g: GameState, who: string, capability: Capability): ActionResult | null {
  return canOperate(g, who, capability)
    ? null
    : { ok: false, reason: `相棒は${capability}担当ではありません` };
}

function actionCapability(id: string): Capability | null {
...
function cookDuration(g: GameState, who: string | null, stationId: string): number {
...
function chopDuration(g: GameState, who: string | null, stationId = 'board'): number {
  const profile = staffProfile(g, who);
  const multiplier = Number(equipmentDurationMultiplier(g.equipment, stationKind(stationId)));
  const equipmentFactor = Number.isFinite(multiplier) && multiplier > 0 ? multiplier : 1;
  return Math.round(
...
const WARMER_SLOT: StationSlot = { name: '保温台', x: 680, y: 390, dx: 0, dy: 82 };
const EXPANSION_SLOTS: Record<string, StationSlot> = {
...
export function roomShell(
  tier: number,
  expansion: number,
): { right: number; width: number; center: number } {
...
function slotInBounds(slot: StationSlot, bounds: KitchenBounds): boolean {
  return (
  ...
  const normalizedStaffState = Object.fromEntries(
    initialHired.map((id) => {
      ...
      const worked = Number.isSafeInteger(supplied.worked)
        ? Math.max(0, Math.min(profile.maxConsecutive - 1, supplied.worked as number))
        : 0;
      const rest = Number.isSafeInteger(supplied.rest)
        ? Math.max(0, Math.min(profile.restShifts, supplied.rest as number))
        : 0;
...
export function stationAt(
  g: GameState,
  who?: string | null,
): { id: string | null; dist: number; inReach: boolean } {
  const e = actor(g, who);
...
export function movePlayer(
  ...
  ay: number,
  step: number,
  movementMode: MovementMode = 'screen',
): void {
...
export function dash(g: GameState, who = 'human'): boolean {
  const e = actor(g, who);
  const profile = staffProfile(g, who);
...
function resetHeat(st: StationState): void {
  st.state = 'idle';
  st.busyUntil = 0;
  st.by = null;
  st.startedAt = 0;
...
export function burnGraceMs(g: GameState, stationId: string): number {
...
function boostAvailable(g: GameState, st: StationState): boolean {
  if (g.practice || !levelConfig(g.level).boostEnabled) return false;
  if (!(st.state === 'chopping' || st.state === 'cooking') || st.boosted || !st.duration)
    return false;
  const progress = (g.time - (st.startedAt ?? 0)) / st.duration;
  return progress >= BOOST_MIN && progress <= BOOST_MAX;
}

function tryBoost(g: GameState, who: string, st: StationState): ActionResult {
  ...
  st.busyUntil = Math.min(
    st.busyUntil ?? 0,
    g.time + Math.max(350, Math.round(((st.busyUntil ?? 0) - g.time) * 0.45)),
  );
  st.boosted = true;
  st.quality = true;
...
function stationIdsOfKind(g: GameState, kind: string): string[] {
  return activeStationIds(g).filter((id) => stationKind(id) === kind);
}

function crewIds(g: GameState): string[] {
  return Array.isArray(g.duty) ? g.duty : Object.keys(g.crew ?? {});
}

export function handoffOption(g: GameState): HandoffOption | null {
  ...
  const partner = crewIds(g)
    ...
    .sort((a, b) => {
      ...
      const cb = actor(g, b);
      return (
        Math.hypot(human.x - (ca?.x ?? 0), human.y - (ca?.y ?? 0)) -
        Math.hypot(human.x - (cb?.x ?? 0), human.y - (cb?.y ?? 0))
      );
    })[0];
...
export function handoff(g: GameState, partner: string): ActionResult {
...
function otherCrew(
  g: GameState,
  who: string,
  predicate: (id: string, e: Actor | null) => boolean,
): boolean {
  return crewIds(g).some((id) => id !== who && predicate(id, actor(g, id)));
}

function stationReserved(g: GameState, stationId: string, who: string): boolean {
  return (
    who !== 'human' &&
...
function actionReserved(g: GameState, actionId: string, who: string): boolean {
  return otherCrew(g, who, (_id, e) => e?.intent?.id === actionId);
}

function canFetchTomato(g: GameState, who: string): boolean {
...
function plateDemand(g: GameState): number {
...
function plateSlotsUsed(g: GameState, who: string): number {
...
function canFetchPlate(g: GameState, who: string): boolean {
  return plateDemand(g) > plateSlotsUsed(g, who) && !actionReserved(g, 'fetch_plate', who);
}

export function interact(g: GameState, who: string | null, stationId: string): ActionResult {
  syncConfig(g);
  normalizeDuty(g);
  ...
  const success = (action: string): ActionResult => ({ ok: true, action });
  const boostStation =
  ...
  switch (kind) {
    ...
    case 'pot':
      ...
      if (st.state === 'idle' && e.carrying === 'chopped') {
        ...
        if (denied) return denied;
        e.carrying = null;
        startHeat(g, who, stationId, st, e.quality ?? false);
        e.quality = false;
      ...
      if (st.state === 'ready' && e.carrying === 'plate') {
        ...
        if (denied) return denied;
        e.carrying = 'soup';
        e.quality = Boolean(e.quality) || Boolean(st.quality);
        resetHeat(st);
    ...
    case 'serve': {
      const denied = capabilityFailure(g, who, 'serve');
      if (denied) return denied;
      if (!e.carrying || !RECIPES[e.carrying])
        return { ok: false, reason: '完成した料理が必要です' };
      advance(g);
...
export function discard(g: GameState, who?: string | null): boolean {
  const e = actor(g, who);
...
export function advance(g: GameState, elapsed = 0): void {
  syncConfig(g);
  ...
  for (const [id, st] of Object.entries(g.stations)) {
    ...
    if (kind === 'board') {
      if (st.state === 'chopping' && g.time >= (st.busyUntil ?? 0)) st.state = 'chopped';
      continue;
    }
    if (kind !== 'pot' && kind !== 'grill') continue;
    if (st.state === 'cooking' && g.time >= (st.busyUntil ?? 0)) {
      st.state = 'ready';
      const grace = burnGraceMs(g, id);
      st.burnAt = grace ? (st.busyUntil ?? 0) + grace : 0;
    }
...
export function buildCandidates(g: GameState, who?: string | null): Candidate[] {
  syncConfig(g);
  normalizeDuty(g);
  const id = resolveWho(g, who);
  ...
  const out: Candidate[] = [];
  const add = (baseId: string, label: string, station: string | null): void => {
  ...
  const needs = (recipe: RecipeId): boolean => player || g.orders.some((o) => o.recipe === recipe);
  if (e && (g.practice || g.time < g.duration)) {
    ...
    if (player) {
      ...
      for (const stationId of active) {
        const work = out.find((c) => c.station === stationId && c.id !== 'interact');
        add(
          `visit_${stationId}`,
          `${stationName(g, stationId)}をタップ：${work ? work.label : '移動のみ（現在の手元では作業できない）'}`,
          stationId,
        );
      }
...
export function repeatsActions(decisions: DecisionRecord[], g: GameState): boolean {
  const cycle = decisions.slice(-4);
  const action = (d: DecisionRecord): string =>
    (d.label ?? d.action ?? '').replace(/^ダッシュして/, '').replace(/（E）$/, '');
  return (
    cycle.length === 4 &&
...
export function cookingAdvice(
  g: GameState,
  cands: Candidate[] = buildCandidates(g, 'human'),
): { instructions: string; hint: string } {
  const held = g?.human.carrying;
  const ordered = (recipe: RecipeId): boolean => g?.orders.some((o) => o.recipe === recipe);
  const plating = g
    ? activeStationIds(g).filter((id) => {
        const recipe = RECIPE_OF_KIND[stationKind(id)];
        return (
          recipe &&
          ordered(recipe) &&
...
export function buildQuestions(cands: Candidate[], who = 'ai') {
  return {
    next_action: {
      type: 'choice',
      instructions:
...
function actorObservation(e: Actor | null) {
  return e
    ? {
        carrying: e.carrying,
        x: Math.round(e.x),
...
function staffObservation(g: GameState, id: string) {
...
function stationObservation(g: GameState, id: string, active: boolean) {
...
export function observe(g: GameState, policy: string | null | undefined, who?: string | null) {
  syncConfig(g);
  const id = resolveWho(g, who);
  ...
  return {
    ...
    pot_burn_seconds:
      g.stations.pot.state === 'ready'
        ? Math.max(0, Math.ceil(((g.stations.pot.burnAt ?? 0) - g.time) / 1000))
        : null,
    grill_burn_seconds:
      g.stations.grill.state === 'ready'
        ? Math.max(0, Math.ceil(((g.stations.grill.burnAt ?? 0) - g.time) / 1000))
        : null,
    burned: g.burned,
    quality: {
...
export function rulePick(
  ...
  const cookware = activeStationIds(g).filter((station) => {
    const recipe = RECIPE_OF_KIND[stationKind(station)];
...
export function actionHint(g: GameState, id: string): string {
  ...
  if (id === 'plates') {
    ...
    if (item && RECIPES[item]) return '完成した料理を持っています';
    return 'お皿を取る';
  }
  if (kind === 'board') {
...
export function kitchenHasWork(g: GameState): boolean {
  return (
    g.stock !== 0 ||
