...
import { trainingMultiplier } from './training.ts';
import type { ActionResult, GameState, Intent, MovementMode } from './types.ts';

/** Inputs and side-effect hooks the caller supplies for one step. */
export interface TickHost {
  ...
  onTargetArrive: () => void;
  onHumanArrive: (intent: Intent) => void;
  onRecord: (who: string, result: ActionResult) => void;
}
...
export function tickWorld(
  ...
  delta: number,
  host: TickHost,
): { missed: boolean; finished: boolean } {
  ...
  if (host.ax || host.ay) {
    ...
    if (station && moveToward(g.human, station.x, station.y, step)) host.onTargetArrive();
  }
