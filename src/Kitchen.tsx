...
import { WorldLabel, Typography, prepareFont, resetFont } from './Surface.tsx';
import SceneUI from './SceneUI.tsx';
import { compactControls } from './ui.ts';
...
import { entranceDuration, entranceHeight } from './entrance.ts';
import { STAFF } from './staff.ts';

const world = (x: number, y: number, height = 0): [number, number, number] => [
  (x - 450) / 65,
  height,
  (y - 270) / 65,
];
const FLOOR_ROWS = 8;
const FLOOR_COLUMNS: Record<number, number> = Object.freeze({ 1: 14, 2: 16, 3: 20 });
...
function Floor({ level, expansion = 1 }: { level: number; expansion?: number }) {
  ...
  return (
    <instancedMesh ref={tiles} args={[undefined, undefined, FLOOR_MAX_TILES]} receiveShadow>
...
function Steam({ active }: { active: boolean }) {
  ...
  useFrame(({ clock }) => {
    ...
    group.current.children.forEach((puff, i) => {
      ...
      ((puff as THREE.Mesh).material as THREE.MeshBasicMaterial).opacity = (1 - t) * 0.45;
    });
  });
  return (
...
function Station({ id }: { id: string }) {
  ...
  const progress =
    working && st.duration ? clamp01(1 - ((st.busyUntil ?? 0) - g.time) / st.duration) : 1;
  const burnDeadline = st.burnAt ?? 0;
  ...
  const stockCount = stockFinite ? Math.max(0, Math.floor(g.stock ?? 0)) : null;
...
function Chef({ who }: { who: string }) {
  ...
  useFrame(() => {
    ...
    if (!a || !root.current || !body.current || !leftArm.current || !rightArm.current) return;
  ...
  return (
    <group ref={root} position={world(actor.x, actor.y)}>
      ...
      <WorldLabel
        position={[0, 2.08, 0]}
        visible={graphicsReady && phase !== 'ready'}
        text={human ? 'あなた' : (STAFF[who].name.split('の').at(-1) ?? who)}
        width={84}
...
export default function Kitchen() {
  ...
  const rendererDisposeRef = useRef<(() => Promise<void>) | null>(null);
    ...
    <GraphicsBoundary key={graphicsKey} onRetry={retryGraphics}>
      {error ? (
        ...
        <Canvas
                ...
                disposal ??= Promise.resolve()
                  .then(() =>
                    instance.hasInitialized()
                      ? instance.dispose()
                      : (instance.backend as unknown as { dispose: () => void }).dispose(),
                  )
                  .catch(() => {
          ...
          onCreated={({ gl }) => {
            if (gl === (rendererRef.current as unknown))
              useKitchen.setState({
                backend: 'WebGPU',
                ready: false,
              });
