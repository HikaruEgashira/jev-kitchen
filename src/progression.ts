...
function progressBetween(level: number, start: number, end: number): number {
  return Math.min(1, Math.max(0, (level - start) / (end - start)));
}

function recipeMix(level: number): Readonly<{ dish: number; soup: number; roast: number }> {
...
function quota(level: number): number {
  if (level === 1) return 1;
  if (level === 2) return 7;
...
function orderWindowMs(level: number): number {
...
function unlockLabel(level: number): string {
  if (level >= 11 && level <= 30)
...
export function levelConfig(level: number = FIRST_LEVEL): LevelConfig {
