...
- 操作設定はThreeボタンで、カメラの自動・追従・全体と、キーボードの画面基準・マス目基準を選ぶ。縦持ちの自動カメラはプレイヤーを追従し、角度は固定する。CSSと依存パッケージは追加しない。
- `29fecf7`ベースの変更で98 tests、typecheck、check（44 files／lint28、警告なし）、build、diff-checkがpass。Kitchen chunkはgzip565.96kB、フォントatlasは非圧縮2,871,100B。既知のchunk警告は継続する。配信前に並行作業の`jev-bench`がmainへ入ったため、最新mainとの統合・再検査を要する。
...
- 配信前の100% versionは`a267e2a0-5b70-4332-973a-159417bdec2c`。復旧コマンドは`pnpm exec wrangler rollback a267e2a0-5b70-4332-973a-159417bdec2c`。WebGPU専用とCloudflare Accessの保護は維持する。
