...
## 2026-09-20 診断内のオートモードとリセット

- 診断にオートモードの切替を追加し、リセットを設定の先頭画面から診断へ移した。リセットの2段階確認は維持する。
- 本編の現在のレベル・所持金・保存・音・相棒の設定を保ち、Benchと共通のJev判断ループでプレイヤーの操作と開店準備を行う。オフ・リセットで保留中の判断を中止し、メニュー表示や一時停止をまたいだ応答は破棄する。オート中の履歴・反復警告もヒントへ表示する。
- 209テスト、型検査、lint・整形、フォント収録検査、production buildが成功した。実ブラウザでは診断の配置、オン・オフの表示、リセット確認の表示を確認した。
- 実APIでのオート操作は未確認。判断・調理・営業間の準備・保存・停止と応答破棄はAPI応答を模擬したテストで検証した。

## 2026-09-20 共有ヒント・JevのLv30到達・最後の5秒のカウント音

- アオの役割表記を「店長」に揃え、退職時期や再登場確率を画面の説明から除いた。ヒントは採用先・育成順・次の調理を指定せず、推奨仕入れ、売値、注文、レシピ、費用と効果を人間とJevに共通で示す。
- `jev-bench-v4`は判断段階ごとに文脈を絞り、直近6判断と反復警告を渡す。警告による行動の置き換えや、低confidenceだけを理由にした再質問は行わない。勤務の見通しには、本編の休養処理で求めた翌営業の出勤可能人数・加熱担当人数を表示する。
- 採用候補に仕入れ・配置込みの予定残金を表示した。Lv6のノルマを6皿、Lv10〜30を7〜12皿とし、サラダ・スープ・グリルの工程数の差を反映した。固定ルールのNPCは、使用中の鍋やグリルを待つ間、注文に使える切った食材を保持する。
- 残り5・4・3・2・1秒に短いカウント音を1回ずつ鳴らす。一時停止・ミュート・時間制限のない練習では鳴らさない。通常の`tick`を通す自動テストで発音回数と除外条件を検証した。
- 最終の加速試行は2回ともLv30をクリアした。`9cc17d7`は2,753 call・Lv30で15/12皿、ゲームコードが同一の`05ba610`は2,888 call・17/12皿で、APIエラーは両方0件。採用・育成・設備強化・増設・厨房拡張をJevが選択した。通信待ちと90秒のゲーム時間は維持し、移動・待機の実時間だけを省いた。通常速度の試行は依頼によりLv23クリア後に中断し、今回の到達性判定には含めない。[測定記録](bench-v4-campaigns.json)に失敗・中断も残す。2回の成功を成功率の保証とはしない。
- Bench終了時に使用済みticketのキャッシュを解放した。本番で12 callの試行を2回続け、両方がLv1クリア・100点として別々に順位へ登録されることを確認した。共有レシピの表示、ヒントの開閉と営業終了も本番で確認し、ブラウザのエラーログは0件だった。音そのものの聴取確認は行っていない。
- 統合時点の206テスト、型検査、lint・整形、フォント収録検査、production buildが成功し、`d0e6ed9`のGitHub ActionsとWorkers Buildsはともに成功した。行動指定を含むv3のLv30クリアや固定入力の正答率は、v4の到達性の証拠に含めない。

## 2026-09-20 主要ボタンの拡大・横向きダイアログの下部操作バー・Eの作業台優先・厨房の6%下降と俯瞰の中央補正配信

- `src/ui.js`の主要操作ボタンを44pxから68pxへ広げ、`開店`の文字も拡大した。横向きダイアログは右側の操作レールを廃止し、`閉じる`などを含む操作グループをシート下部の横並びバーへ置いた。縦画面は68px、仕入れ画面は56px、高さの足りない横向き画面は48〜68pxへ自動調整する。シート高を拡大し、ヒント本文・診断の送信説明・結果の警告の位置を追随させた。
- `handoffOption`（`src/model.js`）は手が届く作業台がある間は`null`を返す。Eは作業台の作業を優先し、品を渡すには作業台から離れる。ベンチの候補列と実プレイが同じ判定を通る。
- `moveCamera`（`src/camera.js`）に`SCENE_DROP=0.06`を追加し、厨房の3Dレンダーを画面高の6%だけ下げた。当初`camera.setViewOffset`で実装したが、`ScreenSpace`のUI看板はメインカメラの変換をコピーして同じカメラで描かれるため、UIも一緒に下がった。カメラリグを`translateY`で持ち上げる方式へ直し、UIは画面位置を保つ。ポインタ判定はカメラ変換に追随する。
- 俯瞰（非追従）の注視点を作業台の範囲中心から部屋の見た目の中心へ移した。部屋の外形は`roomShell`（`src/model.js`）を唯一の正とし、`Room`メッシュと`cameraFraming`が共有する。これで厨房が画面中央に揃う（従来は2%強右寄り）。追従中は従来どおりプレイヤー中心で、左右の補正はしていない。
- 出荷ゲート: 205テスト、typecheck、check、buildがpass。UIの44px以上・シート内・非重複を縦横で回帰検証する。実ブラウザでの目視は未実施。
- commit `492b4fd`（UI・E）、`8d40861`（下降の初回）、`cd3cadb`（カメラリグ方式への修正）、`ffc8aa3`（部屋中心へ）をmainへpush。本番100% versionは`96e51a16-9ec4-4073-908a-7e991b6c5277`。`GET /`は200で配信bundleはローカルbuildと一致（`index-DnS_WLc6.js`）。
- 復旧先は100% version `af9ee6d5-1fc1-49c7-b4de-60d2cf70f158`（この一連の変更の直前）。下降だけ戻す場合は`pnpm exec wrangler rollback 696e2038-9195-46bd-9464-308b6bc3f5ef`。UIだけ戻す場合は`src/ui.js`を`git revert`する。
