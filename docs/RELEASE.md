...
- `moveCamera`（`src/camera.js`）に`SCENE_DROP=0.06`を追加し、`camera.setViewOffset`で厨房の3Dレンダーを画面高の6%だけ下げた。UI（ScreenSpace）とポインタ判定は対象外。
- 出荷ゲート: 204テスト、typecheck、check、buildがpass。UIの44px以上・シート内・非重複を縦横で回帰検証する。実ブラウザでの目視は未実施。
- commit `492b4fd`（UI・E）と`8d40861`（下降）をmainへpush。本番100% versionは`e59ebc1c-7d50-4518-9521-7df62c7c6701`（2026-09-19T16:47:48Z）。`GET /`は200で配信bundleはローカルbuildと一致（`index-BZ-w6jrO.js`）。
- 復旧先は100% version `af9ee6d5-1fc1-49c7-b4de-60d2cf70f158`（この3変更の直前）。下降だけ戻す場合は`pnpm exec wrangler rollback 696e2038-9195-46bd-9464-308b6bc3f5ef`。UIだけ戻す場合は`src/ui.js`を`git revert`する。
