...
- `/bench`でJev Choice互換モデルをプレイヤーとして実行する。固定ルールのハル、同じ財布・在庫・営業時間で複数試行し、到達レベル・クリア数・応答時間と判断履歴をJSONに記録する。
- 通常プレイの保存を抑止し、応答時・到着時に合法性を検証する。モデルエラー・判断上限・中断を通常の未達と区別し、モデルエラー時の代替判断と自動再試行は行わない。
- 接続先はWorkerの`BENCH_ENDPOINTS`に登録したHTTPS endpointのみ。URL・トークンをブラウザへ返さず、既存のサイズ制限・timeout・Access保護を維持する。
- 自動検査: 80 tests、typecheck、format/lint、build、Wrangler dry-runがpass。通常プレイの回帰、保存の保全、候補外応答、遅延応答、中断、複数試行の初期化を含む。
- ローカルChrome/WebGPUと実Jev（TypeSafe API）でLv1ノルマ達成、90秒後のLv2への進行を確認。Lv2の6/8皿・残り28秒まで観測したが、その後ブラウザ接続が切れたため、この試行の最終結果は未採取。
- 配信前の復旧候補: `a267e2a0-5b70-4332-973a-159417bdec2c`。本番適用・配信後検証は追記する。
