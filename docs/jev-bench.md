...
営業中のヒントと準備画面の「ヒント」は、Jevの指示と同じ条件分岐から生成する。準備では推奨仕入れ・前回販売数・残在庫・支払後残額・次の注文比率・設備枠を表示する。直近6操作と反復警告も確認でき、狭い画面ではヒントをページ送りする。メニュー表示中は時計とJev判断を停止し、閉じると元の準備・営業へ戻る。
