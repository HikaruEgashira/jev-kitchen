...
`/bench`（`jev-bench-v3`）は、Jev Choice互換のdecision endpointを持つモデルをプレイヤーとして実行する。
...
準備は採用→勤務→仕入れ→投資の順に、一つのChoiceで一つの判断を求める。勤務は合法な出勤者の組合せを`crew_*`から一括選択し、仕入れは`stock_*`／`confirm_stock`で確定して次の準備判断へ進む。支払いは`open_shift`で一括確定する。採用は出勤を意味しない。営業中は手元と盛り付け待ちの食材に応じた短い目的を提示し、注文のない完成品をQで片づける判断を促す。相棒との受け渡しは手元・距離・相手を応答時に再検証する。共通の準備候補には`assign_*`（追加）、`rest_*`（1人解除）、`replace_*`（1人交代）も残すが、試行中は勤務表全体を一度に選ぶことで交代の往復を避ける。各段階で必要な休養予定・次の出勤枠・投資費用をモデルへ渡す。推奨仕入れ数は目安であり、前回販売数・残在庫・単価・持ち越し可否を示して仕入れ量をモデルに委ねる。販売量を増やす命令は与えない。投資段階は給与と仕入れを確保した残額で選択する。設備枠の使用数・上限と、厨房拡張後の枠数を示し、増設できない理由を判断可能にする。設備購入とその取消は別の行動IDにし、連続した判断で意味が逆転しないようにする。料理人は連勤数の少ない担当者を推薦し、別の料理人を休ませる方針を示す。合法な勤務候補は全て残す。
...
TypeSafeの[公式ガイド](https://docs.typesafe.ai/introduction)に沿って判断を分割した。長い指示だけでは投資の省略が残ったため、操作可能な範囲を段階ごとに示す。v1・v2・v3の成績は直接比較せず、同一protocol・revisionで比較する。

## 人間と共有する情報

営業中のヒントと準備画面の「ヒント」は、Jevの指示と同じ条件分岐から生成する。準備では推奨仕入れ・前回販売数・残在庫・支払後残額・次の注文比率・設備枠を表示する。直近6操作と反復警告も確認でき、狭い画面ではヒントをページ送りする。メニュー表示中は時計とJev判断を停止する。閉じた後も営業は一時停止を維持し、再開操作で続ける。
