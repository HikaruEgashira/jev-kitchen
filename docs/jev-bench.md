...
[固定入力](../test/fixtures/bench-decisions.json)は調整用9局面と、別の実時間試行から採った確認用6局面を分ける。2026-09-19の3回反復では、従来文脈が調整用21/27・確認用12/18、縮約文脈が27/27・18/18だった。送信量の平均は約5.7KBから2.0KBに減った。[観測結果](bench-context-evaluation.json)には正誤・confidence・候補数・サイズ・応答時間を残す。これは15局面内の比較であり、Lv30の通し試行とは別の検証である。確認用の採用ケースで、候補に存在しない料理人を正解とした初期の評価誤りを修正して再測定した。
