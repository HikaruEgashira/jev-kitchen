...
判断の目的と状態説明を分け、操作の効果は`state.situation`に置く。営業中は現在の注文のレシピ工程、仕入れ時は料理の売値も共有する。工程は人間のヒントと同じ文字列を使い、特定の次行動を推奨しない。採用候補には採用費・現在の仕入れ・配置を反映した予定残金、設備候補には増設台数や強化の効果を示し、人間の準備画面と同じ計算を使う。準備の履歴は現在の段階の直近6判断に絞り、投資には仕入れ数量や採用時の履歴を混ぜない。
