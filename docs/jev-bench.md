...
全モデルが通常プレイと同じ移動速度・調理・採用・仕入れ処理を使う。画面は左に厨房、右に状態と設定、下に判断ログを置くRTA配置。
...
- 設定はfrequency（秒あたりのcall上限、既定1、0.1〜10）と最大call数（既定1,000、1〜10,000）。1回の開始で1試行する。目標レベルは設けず、ノルマ未達・call上限・全100レベルクリアまで進む。
...
通常の保存データは更新しない。中断・再開で時計を止める操作や未達後のリトライは評価の停止・新しい試行として扱う。結果はタブ内に保持し、JSONをダウンロードして保存する。
