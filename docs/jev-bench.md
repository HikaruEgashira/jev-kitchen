...
全モデルが通常プレイと同じ移動速度・調理・採用・仕入れ処理を使う。画面は左にSIDEKICK本体と同一のKitchen・SceneUI、右上に判断履歴、右下に実行条件・開始操作・状態と今回クリアしたレベル・所持金・スコアを置く。SIDEKICKの配色・文字・操作部品を使い、1画面に収める。過去試行の表は表示しない。
