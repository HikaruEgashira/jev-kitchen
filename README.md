...
- Lv2以降は各90秒。Lv2〜4は9皿、Lv5〜7は10皿、Lv8〜10は11皿、以降は計算式で最大16皿へ難化する。注文猶予は30秒から20秒へ短くなる。星は600／1800／3600点の得点評価で、ノルマ判定とは別に扱う。
- Lv3クリアから応募者を最大3名提示し、1人を採用するか見送る。同時出勤はLv8で2人、Lv12で3人、Lv16で4人へ増える。採用費とは別に出勤者の給与を毎営業支払い、休養中は配置できない。
...
- Lv2以降はトマト在庫が有限で、料理1皿に1個使う。余剰在庫は次営業へ繰り越し、営業間の仕入れは1個8コイン。採用と仕入れは初期180コインの同じ財布で扱い、次ノルマ以上の在庫がないと開店できない。
...
- 設備・採用価格は固定の貯蓄単位90コインを基準に算出する。まな板増設270、厨房拡張720／1,080、採用270〜1,440。価格はレベル上昇で値上がりせず、仕入れ・給与分を残して貯蓄する。
- 失敗時は同条件で再挑戦、開店準備の見直し、前ステージへの巻戻しを選ぶ。財布・在庫・雇用・疲労・設備・厨房拡張・配置をまとめて復元し、支払い・報酬の二重計上や応募者の再抽選を防ぐ。履歴のない選択肢は使えない。
- 営業開始時のレベル・財布・在庫・雇用・出勤・疲労・設備・配置と、一段前の巻戻し先を端末に保存する。再読み込み後の「開店」で営業を最初から再開する。営業途中の進捗と未確定の開店準備は保存しない。準備の見直し中に再読み込みした場合は、前営業の開始時点へ戻る。
...
プレイヤーとしてどこまでクリアできるかを試行する。モデル・frequency（秒あたりのcall上限）・最大call数（既定1,000）を指定し、
