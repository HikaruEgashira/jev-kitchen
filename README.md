...
- 操作はWASD／矢印で移動、Eで作業、Shiftでダッシュ、Qで片づけ。スマホは作業台の3Dボタン。設定でカメラ・移動基準・バックグラウンドモードを切り替える。
...
- ランキングはベンチのフルキャンペーンを対象に、run終了時にサーバが同じseedと決定列から再実行して確定する。設計は[公開API仕様](docs/public-api.md)。
