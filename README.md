...
- Lv5でスープ鍋、Lv10でグリル、Lv15で手動ブースト、Lv20で焦げ管理、Lv25で連勤と休み、Lv30でラッシュを解放する。Lv31以降は新設備を増やさず、ノルマ・注文猶予・商品比率で難化する。Lv100クリアで完走。
- クリア後は未採用の応募者から最大3名を提示し、1人を採用するか見送って貯金する。同時出勤の上限は相棒1人から、Lv10で2人、Lv20で3人、Lv30で4人へ増える。出勤者の給与を毎営業支払い、Lv25以降は連勤上限と休養日数も見て配置する。
...
- 失敗時は「同じ条件で再挑戦」「仕入れ・採用から見直す」「前のステージへ戻る」を選ぶ。財布・在庫・雇用・疲労をまとめて復元し、支払い・報酬の二重計上や応募者の再抽選を防ぐ。履歴のない選択肢は使えない。
- 営業開始時のレベル・財布・在庫・雇用・出勤・疲労と、一段前の巻戻し先を端末に保存する。再読み込み後の「開店」で営業を最初から再開する。営業途中の進捗と未確定の採用・仕入れは保存しない。準備の見直し中に再読み込みした場合は、前営業の開始時点へ戻る。
- 作業台のクリック／タップで移動し、キーボードはWASD／矢印で移動、Eで作業、Shiftでダッシュ、Qで手元を片付ける。移動はマス目基準を既定とし、操作設定で画面基準にも切り替えられる。右上は出勤者の閲覧専用リストとし、方針入力・判断方式・能力の個別変更は設けない。
- 設定のカメラは自動・追従・全体から選ぶ。自動は縦持ちでプレイヤーを大きく追い、横持ちで厨房全体を映す。カメラの角度は固定し、動きを減らす設定では切替の補間を止める。開店前と結果画面は全体表示に戻る。
...
[`/bench`](https://jev-kitchen.egahika.dev/bench)で、Jev Choice互換のAIモデルが
プレイヤーとしてどこまでクリアできるかを試行する。
モデル・frequency（秒あたりのcall上限）・最大call数（既定1,000）を指定し、到達レベル、クリア数、判断回数、
平均／p95応答時間を比較する。結果と各判断の記録はJSONで保存できる。

全試行はLv1・初期120コインからハルの給与12を支払った108コイン・見習いのハルで開始する。相棒は固定ルールで動く。API待ち時間も90秒の営業に含め、
合法な候補と通常の移動・調理処理を使う。通常プレイのセーブデータは更新しない。
採用・相棒の配置・0〜99個の仕入れもモデルが選ぶ。自由移動・ダッシュ・仕上げ・返却・廃棄を通常の人間操作と同じ処理で実行する。
Jev障害を固定ルールで補わず、エラーとして記録する。タブを隠すと試行は中断する。

Jevは既存の秘密鍵で利用可能。他モデルの追加とAPI仕様は[jev-bench接続仕様](docs/jev-bench.md)を参照。
結果はタブ内だけに保持され、再読込みで消える。公開ランキング・成績のサーバー検証は設けない。
