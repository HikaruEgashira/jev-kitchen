...
右枠に今回クリアしたレベルと所持金・スコアを並べる。結果と各判断の記録はJSONで保存できる。

全試行はLv1・120コイン・見習いのハルから開始する。相棒は固定ルールで動く。API待ち時間も90秒の営業に含め、
合法な候補と通常の移動・調理処理を使う。通常プレイのセーブデータは更新しない。
採用・相棒の配置・0〜99個の仕入れもモデルが選ぶ。自由移動・ダッシュ・仕上げ・返却・廃棄を通常の人間操作と同じ処理で実行する。
Jev障害を固定ルールで補わず、エラーとして記録する。中断やタブ非表示では一時停止し、同じゲーム状態とcall数で再開できる。
