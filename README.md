...
- 開店準備でまな板・鍋・グリルを各2台まで増設し、各Lv3まで改良できる。改良は調理時間を8%ずつ短縮する。Lv20から導入できる保温台は完成後の焦げ猶予を延ばす。厨房は3段階で、追加設備枠が1／2／4枠へ増え、拡張に合わせて面積も広がる。
- 設備の移動は空き区画への移設か設備同士の入替で行う。配置・投資をプレビューしてから、採用・仕入れ・給与と一括確定する。レベルで解放される基本の鍋・グリルは追加購入不要。
- 設備・採用価格はLv2の標準手残り90コインを基準に算出する。まな板増設270、厨房拡張720／1,080、採用270〜1,440。価格はレベル上昇で値上がりせず、仕入れ・給与分を残して貯蓄する。
...
右に判断履歴、その下に実行条件・開始操作と今回クリアしたレベル・所持金・スコアを並べる。結果と各判断の記録はJSONで保存できる。
...
Jev障害を固定ルールで補わず、エラーとして記録する。中断では一時停止し、タブ非表示の扱いは「バックグラウンドモード」に従う。同じゲーム状態とcall数で再開できる。
