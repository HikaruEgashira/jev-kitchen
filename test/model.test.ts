...
import { nextStaffState, payroll, staffAvailable, STAFF, staffPerformance } from '../src/staff.ts';
import type { GameState, RecipeId, StationStateName } from '../src/types.ts';
...
test('a second board allows two cooks to fetch ingredients in parallel', () => {
  ...
  g.crew.helper.intent = { id: 'fetch_tomato', station: 'crate', label: 'トマト', startedAt: 0 };
...
test('nearby handoffs preserve items and quality and reject stale, busy or absent partners', () => {
  ...
  Object.assign(g.human, {
    ...
    carrying: 'soup',
    quality: true,
    intent: { id: 'serve', label: '配膳する', startedAt: 0 },
  });
  Object.assign(g.crew.helper, {
    x: 520,
    y: 300,
    intent: { id: 'fetch_tomato', label: 'トマト', startedAt: 0 },
  });
...
test('unknown actions are never feasible', () => {
  const g = createGame({ level: 3, stock: 99 });
  assert.equal(isFeasible(g, { id: 'fly_to_moon', label: '月へ' }), false);
...
test('all 100 levels follow a bounded curve and reproducible recipe proportions', () => {
  ...
  for (let level = 1; level <= MAX_LEVEL; level++) {
    ...
    for (const recipe of Object.keys(counts) as RecipeId[]) {
...
test('staff profile changes AI preparation without changing the candidate contract', () => {
  ...
  assert.equal(observe(g, null).staff!.id, 'chef');
  assert.deepEqual(observe(g, null).staff!.capabilities, ['prep', 'cook']);
  assert.equal(observe(g, null).staff!.decision_interval_ms, 900);
  g.level = 10;
...
test('crew actors obey the level cap and reserve exclusive work independently', () => {
  ...
  actor(g, 'helper')!.intent = { id: 'chop', station: 'board', label: '切る', startedAt: 0 };
  actor(g, 'sous')!.carrying = 'tomato';
  assert.equal(isFeasible(g, { id: 'chop', station: 'board', label: '切る' }, 'sous'), false);
...
test('crew reservations prevent duplicate tomato and plate pickups', () => {
  ...
  actor(g, 'helper')!.intent = { id: 'fetch_plate', station: 'plates', label: '皿', startedAt: 0 };
...
test('a moved station survives the next unlock without colliding with the new station', () => {
  ...
  assert.equal(stationInfo({ ...before, level: 5, equipment, layout: after }, 'board')!.x, 930);
});
...
test('menu prices are paid per dish independently of score bonuses', () => {
  ...
  for (const [recipe, price] of [
    ...
    const before = g.cash;
    g.human.carrying = recipe;
    g.orders = [{ id: 0, recipe, deadline: 30000, duration: 30000 }];
    assert.equal(interact(g, 'human', 'serve').ok, true);
...
test('cooks retain useful ingredients while occupied cookware becomes available', () => {
  for (const [station, recipe, action] of [
    ['pot', 'soup', 'cook'],
    ['grill', 'roast', 'grill'],
  ] as [string, RecipeId, string][]) {
    for (const state of ['cooking', 'ready'] as StationStateName[]) {
...
test('blocked cooks can discard, clean burnt cookware and start cooking again', () => {
  for (const staffId of ['chef', 'sous']) {
    for (const station of ['pot', 'pot2', 'grill', 'grill2']) {
      ...
      const recipe: RecipeId = kind === 'pot' ? 'soup' : 'roast';
      const g = createGame({
        level: 20,
        stock: 4,
        staffId,
        equipment: { [kind]: { count: 2, level: 1 } },
      });
...
test('hand recovery respects usable stations, crew reservations and stale decisions', () => {
  ...
  g.crew.sous.intent = { id: 'cook_pot2', station: 'pot2', label: '煮る', startedAt: 0 };
  const recovery = rulePick(g, buildCandidates(g))!;
  ...
  assert.ok(!buildCandidates(g).some((c) => c.id === 'discard'));
  g.time = g.duration;
  assert.equal(isFeasible(g, { id: 'discard', station: null, label: '捨てる' }), false);
...
test('the rule companion can finish soup and stale station targets are rejected', () => {
  ...
  assert.equal(isFeasible(g, { id: 'fetch_tomato', station: 'serve', label: 'トマト' }), false);
...
test('practice mode teaches one salad without timers or pressure', () => {
  ...
  assert.equal(interact(g, 'human', 'crate').ok, true);
  assert.equal(discard(g, 'human'), true);
  assert.equal(observe(g, null).seconds_left, null);
});
...
test('every sidekick has a unique theme color and relative performance bars', () => {
  ...
  const ratio = (id: string, key: string) =>
    staffPerformance(id).find((bar) => bar.key === key)!.ratio;
