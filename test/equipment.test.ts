...
import { RECIPES, STOCK_PRICE, levelConfig, createGame, recommendedStock } from '../src/model.ts';
...
test('equipmentState fills and clamps createGame-compatible state without mutating input', () => {
  ...
  assert.deepEqual(equipmentState(undefined), DEFAULT_EQUIPMENT);
...
test('quoteEquipment applies valid repeated purchases atomically and non-destructively', () => {
  const current = equipmentState(undefined);
