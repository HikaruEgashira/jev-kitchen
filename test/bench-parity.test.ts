...
import { buildCandidates, createGame } from '../src/model.ts';
import { useKitchen } from '../src/game.ts';
import { preparation, screen, screenContext } from '../src/ui.ts';
import { PREPARATION_ACTIONS, preparationCandidates } from '../src/benchmark.ts';
import type { Candidate, StoreState, ViewState } from '../src/types.ts';
...
test('screenContext keeps a control id only when the actor can choose it', () => {
  const { g, state } = fixture();
  const cases: { state: StoreState; view: ViewState; actions: Candidate[] }[] = [
