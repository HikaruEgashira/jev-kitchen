...
import { cameraFraming, moveCamera } from '../src/camera.ts';
import { createGame, kitchenBounds } from '../src/model.ts';
    ...
    for (const level of [1, 5, 10, 20, 30, 100]) {
      ...
      for (const x of [bounds.minX, bounds.maxX]) {
        for (const y of [bounds.minY, bounds.maxY]) {
          ...
          // No viewport on purpose: the scene-drop offset is skipped, as before.
          moveCamera(camera, target, framing, 1 / 60, true, 0, 0);
...
test('following eases without overshoot or camera rotation, and reduced motion snaps', () => {
  ...
  const target = new Vector3();
  moveCamera(camera, target, cameraFraming(g, 'playing', 'auto', 320, 568), 0, true, 0, 0);
  const rotation = camera.quaternion.clone();
  const before = target.clone();
  g.human.x += 200;
  const framing = cameraFraming(g, 'playing', 'auto', 320, 568);
  moveCamera(camera, target, framing, 1 / 60, false, 0, 0);
  ...
  moveCamera(camera, target, overview, 1 / 60, true, 0, 0);
