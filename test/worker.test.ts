...
import worker, { GameStore, type Env } from '../src/worker.ts';
...
interface SeenRequest {
  url: string;
  body: { model: string; questions: unknown };
...
const env = (): Env => ({
  ...
  AI: {
    async run(model: string, input: { state: unknown; questions: unknown }) {
...
const ticketFor = (path: string) =>
  path === '/api/bench/decide' || path === '/api/runs/finish' ? benchTicket : playTicket;

const post = (path: string, body: unknown, ticket: string = ticketFor(path)) =>
...
test('benchmark registry exposes labels only and forwards only to a server-registered endpoint', async (t) => {
  ...
  const fetch = t.mock.method(globalThis, 'fetch', async (url: FetchInput, options?: FetchInit) => {
...
for (const direct of [false, true]) {
  test(`decide validates and projects next_action (${direct ? 'direct' : 'binding'})`, async (t) => {
    ...
    const testEnv: Env = { TICKET_SECRET: SECRET, AI: { run: async () => raw } };
    if (direct) {
...
test('the run Durable Object stores per-run chains and one sorted board', async () => {
  ...
  const board = (await (await call('/board', undefined, 'GET')).json()) as {
    board: Array<{ sid: string }>;
  };
  assert.deepEqual(
