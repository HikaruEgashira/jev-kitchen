...
import { levelConfig, MAX_LEVEL, quotaForLevel } from '../src/progression.ts';
...
test('levelConfig clamps input and returns the full 1..100 campaign', () => {
  ...
  // The runtime clamps non-numeric input; the public signature is numeric.
  assert.equal(levelConfig('not-a-level' as unknown as number).level, 1);
